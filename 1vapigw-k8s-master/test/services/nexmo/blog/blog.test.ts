...
describe('Nexmo', () => {
  describe.each(getHostList(LobTypes.API))('Blog Sanity - %s', host => {
    let request;
    ...
    describe(`${service} Services`, () => {
      describe.each(domains)('domain - %s', domain => {
        test('GET / - Should return 301', async () => {
          ...
          const headers = {
            Host: domain,
            [TRACE_ID_HEADER]: '1vapigw-blog-test-1',
          };

          const response = await request.get(path).set(headers);
          expect(response.status).toEqual(StatusCodes.MOVED_PERMANENTLY);
          expect(response.headers['location']).toMatch('https://developer.vonage.com/blog');
        });
      });
    });
  });
